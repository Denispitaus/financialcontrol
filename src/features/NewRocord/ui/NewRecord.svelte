<script>
	...
	let { JSRecords, monthName } = $props();
</script>

<form onsubmit={clickAddRecord} class="addRecord">
	<Title text="Добавить запись" className="TitleBlock" />
	<div class="addRecordButtons">
		<Button
			text="+ Доход"
			className={ButtonSave ? 'btnRecordGreenActive' : 'btnRecordGreen'}
			onclick={clickGreenBtn}
		/>
		<Button
			text="- Расход"
			className={!ButtonSave ? 'btnRecordReActive' : 'btnRecordRed'}
			onclick={clickRedBtn}
		/>
	</div>
	<Label text="Название категории" className="MainInfoTitle" />
	<Input
		className={ColorInputError ? 'inputRecordError' : 'inputRecord'}
		minlength="1"
		maxlength="14"
		placeholder="Например: Аренда, Еда..."
		type="text"
		bind:value={TitleSave}
	/>
	<Label text="Сумма, ₽" className="MainInfoTitle" />
	<Input
		className={ColorInputErrorNumber ? 'inputRecordError' : 'inputRecord'}
		placeholder="0.00"
		type="number"
		bind:value={NumberSave}
	/>
	<Label text="День месяца" className="MainInfoTitle" />

	<select id="daySelect" name="day" class="inputRecord" bind:value={DaySave}>
		{#each daysRange as day (day)}
			<option value={day}>{day}</option>
		{/each}
	</select>
	<Button type="submit" text="Добавить категорию" className="addCategore" />
</form>
