<script>
	...
	let FreeMoney = $derived(
		MassiveRecord.filter((el) => el.ButtonSave && el.monthName === MonthName).reduce(
			(sum, el) => sum + el.NumberSave,
			0
		)
	);
...
<section class="main">
	<Header monthname={MonthName} {NextMonth} {BackMonth} {VisibleMonth} {checkNowDate} />
	<div class="home">
		<MainInfo
			countCategory={FilterMassiveCategory.length}
			freeMoney={FreeMoney}
			notMoney={NotMoney}
			{MonthName}
		/>
